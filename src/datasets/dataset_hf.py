...
def hf_vars_main(data_root: Path,
    ...
    for fleet_name, fleet_dir in fleet_dirs.items():
        ...
        success = hf_vars_extractor(high_frequency_folder=str(fleet_dir),
                                    ...
                                    perform_imputation=False,      # Make it by default as True
...
def hf_vars_extractor(high_frequency_folder: str,
    ...
    for filename in os.listdir(high_frequency_folder):
        ...
        if missing_req:
            logger.warning(
                f"{filename}: missing required columns {missing_req} - skipping"
            )
            continue
        ...
        data_filtered = (
            ...
            .loc[:, ~data[available_signals].columns.duplicated()]
            .copy()
        )
        ...
        try:
            data_filtered["TIMESTAMP"] = pd.to_datetime(
                data_filtered["TIMESTAMP"], errors="coerce"
            )
            bad_ts = data_filtered["TIMESTAMP"].isna().sum()
            if bad_ts > 0.5 * len(data_filtered):
                logger.warning(
                    f"  {bad_ts:,} TIMESTAMP values failed to parse - check format"
                )
            data_filtered = (
                data_filtered
        ...
        if perform_imputation:
            logger.info("Imputation branch: physics-based")
            df_impute = df.copy()
            
            # Step 1: forward/backward fill for very short gaps (<= 3 steps)
            try:
                max_short_gap = 3
                num_cols_now  = df_impute.select_dtypes(include=[np.number]).columns.tolist()
                df_impute[num_cols_now] = (
                    df_impute[num_cols_now]
                    .ffill(limit=max_short_gap)
                    .bfill(limit=max_short_gap)
                )
                logger.info(f"  Short-gap fill (+/-{max_short_gap} steps) applied")
            except Exception:
                logger.exception("Short-gap fill failed")
                
            # Step 2: time-aware linear interpolation for medium gaps (<= 50 steps)
            try:
                if isinstance(df_impute.index, pd.DatetimeIndex):
                    num_cols_now = df_impute.select_dtypes(include=[np.number]).columns.tolist()
                    df_impute[num_cols_now] = df_impute[num_cols_now].interpolate(
                        method="time", limit_direction="both", limit=50
                    )
                    logger.info("  Time interpolation (limit=50) applied")
            except Exception:
                logger.exception("Time interpolation failed")
                
            # Step 3: physics-based imputation
            try:
                df_impute = data_imputation(df_impute)
            except Exception:
                logger.exception("data_imputation raised an exception - "
                                "proceeding with whatever has been filled so far")
                
            # Restore static vessel geometry columns if somehow dropped
            for static_col in ("IMO", "LBP", "BEAM", "PROPELLER_DIAM"):
                if static_col in df.columns and static_col not in df_impute.columns:
                    df_impute[static_col] = df[static_col]
                    
            logger.info(f"Imputation complete - remaining NaN: {df_impute.isna().sum().sum():,}")
            
            # Keep only main required signals
            cols_to_keep = [c for c in ocean_pulse_signals if c in df_impute.columns]
            for sc in ("IMO", "LBP", "BEAM", "PROPELLER_DIAM"):
                if sc in df_impute.columns and sc not in cols_to_keep:
                    cols_to_keep.append(sc)
            df_impute = df_impute[cols_to_keep]
            
            nan_count = df_impute.isna().sum().sum()
            if nan_count > 0:
                logger.warning(
                    f"Imputation left {nan_count:,} NaN values in main columns - "
                    f"applying dropna() safely on the remaining dataset"
                )
            ...
            if speed_filter_threshold is not None and SignalNames.STW in df_after_speed.columns:
                df_after_speed = df_after_speed[
                    df_after_speed[SignalNames.STW] >= speed_filter_threshold
                ]
                n_dropped = len(df_imputed) - len(df_after_speed)
                logger.info(
                    f"Speed filter (STW >= {speed_filter_threshold} m/s): "
                    f"dropped {n_dropped:,} rows -> {len(df_after_speed):,} remaining"
                )
            else:
                logger.info("  Speed filter: skipped")
        ...
        # ==================================================================
            ...
            if cfd_conditions is not None:
                df_after_cfd = apply_cfd_conditions(df_after_cfd, cfd_conditions)
                n_dropped = len(df_after_power) - len(df_after_cfd)
                logger.info(f"CFD filter: dropped {n_dropped:,} rows -> "
                            f"{len(df_after_cfd):,} remaining"
                        )
            else:
                logger.info("  CFD filter: skipped (no conditions provided)")
...
def data_imputation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if stw_col in df.columns:
        ...
        if mask.any() and {stw_long, stw_trans} <= set(df.columns):
            ...
            df.loc[fill_mask, stw_col] = np.sqrt(df.loc[fill_mask, stw_long] ** 2+ df.loc[fill_mask, stw_trans] ** 2)
